...
int partslist_report_mask(SpacePartslist *UNUSED(spartslist))
{
#if 0
  int report_mask = 0;

  if (spartslist->rpt_mask & PARTSLIST_RPT_DEBUG)
    report_mask |= RPT_DEBUG_ALL;
  if (spartslist->rpt_mask & PARTSLIST_RPT_INFO)
    report_mask |= RPT_INFO_ALL;
  if (spartslist->rpt_mask & PARTSLIST_RPT_OP)
    report_mask |= RPT_OPERATOR_ALL;
  if (spartslist->rpt_mask & PARTSLIST_RPT_WARN)
    report_mask |= RPT_WARNING_ALL;
  if (spartslist->rpt_mask & PARTSLIST_RPT_ERR)
...
{
  //  SpacePartslist *sc = CTX_wm_space_partslist(C);
  //  ReportList *reports = CTX_wm_reports(C);
  //  int report_mask = partslist_report_mask(sc);
  //  Report *report;
...
void PARTSLIST_OT_report_replay(wmOperatorType *ot)
{
  /* identifiers */
  ot->name = "Replay Operators";
  ot->description = "Replay selected reports";
  ot->idname = "PARTSLIST_OT_report_replay";

  /* api callbacks */
  ot->poll = ED_operator_partslist_active;
...
static int select_report_pick_exec(bContext *C, wmOperator *op)
{
  ...
  ReportList *reports = CTX_wm_reports(C);
  const int report_mask = partslist_report_mask(spartslist);
  if (!report) {
...
static int select_report_pick_invoke(bContext *C, wmOperator *op, const wmEvent *event)
{
  ...
  report = partslist_text_pick(spartslist, ar, reports, event->mval[1]);
...
void PARTSLIST_OT_select_pick(wmOperatorType *ot)
{
  /* identifiers */
  ot->name = "Select Report";
  ot->description = "Select reports by index";
  ot->idname = "PARTSLIST_OT_select_pick";
...
void PARTSLIST_OT_select_all(wmOperatorType *ot)
{
  /* identifiers */
  ot->name = "(De)select All";
  ot->description = "Change selection of all visible reports";
  ot->idname = "PARTSLIST_OT_select_all";
...
static int box_select_exec(bContext *C, wmOperator *op)
{
  ...
  ARegion *ar = CTX_wm_region(C);
  ReportList *reports = CTX_wm_reports(C);
  int report_mask = partslist_report_mask(spartslist);
  ...
  report_min = partslist_text_pick(spartslist, ar, reports, rect.ymax);
  report_max = partslist_text_pick(spartslist, ar, reports, rect.ymin);
...
/* ****** Box Select ****** */
void PARTSLIST_OT_select_box(wmOperatorType *ot)
{
  /* identifiers */
  ot->name = "Box Select";
  ot->description = "Toggle box selection";
  ot->idname = "PARTSLIST_OT_select_box";
...
void PARTSLIST_OT_report_delete(wmOperatorType *ot)
{
  /* identifiers */
  ot->name = "Delete Reports";
  ot->description = "Delete selected reports";
  ot->idname = "PARTSLIST_OT_report_delete";
