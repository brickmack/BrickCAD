...
static int update_reports_display_invoke(bContext *C, wmOperator *UNUSED(op), const wmEvent *event)
{
  ...
  timeout = (report->type & RPT_ERROR_ALL) ? ERROR_TIMEOUT : PARTSLIST_TIMEOUT;
  ...
  if (report->type & RPT_ERROR_ALL) {
    UI_GetThemeColorType3fv(TH_PARTSLIST_ERROR, SPACE_PARTSLIST, target_col);
  }
  else if (report->type & RPT_WARNING_ALL) {
    UI_GetThemeColorType3fv(TH_PARTSLIST_WARNING, SPACE_PARTSLIST, target_col);
  }
  else if (report->type & RPT_INFO_ALL) {
    UI_GetThemeColorType3fv(TH_PARTSLIST_INFO, SPACE_PARTSLIST, target_col);
  }
  target_col[3] = 0.65f;
...
void PARTSLIST_OT_reports_display_update(wmOperatorType *ot)
{
  /* identifiers */
  ot->name = "Update Reports Display";
  ot->idname = "PARTSLIST_OT_reports_display_update";
