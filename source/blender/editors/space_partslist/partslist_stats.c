...
#define MAX_PARTSLIST_LEN 512
#define MAX_PARTSLIST_NUM_LEN 16
...
typedef struct SceneStats {
  ...
  char partsliststr[MAX_PARTSLIST_LEN];
} SceneStats;

typedef struct SceneStatsFmt {
  /* Totals */
  char totvert[MAX_PARTSLIST_NUM_LEN], totvertsel[MAX_PARTSLIST_NUM_LEN];
  char totface[MAX_PARTSLIST_NUM_LEN], totfacesel[MAX_PARTSLIST_NUM_LEN];
  char totedge[MAX_PARTSLIST_NUM_LEN], totedgesel[MAX_PARTSLIST_NUM_LEN];
  char totbone[MAX_PARTSLIST_NUM_LEN], totbonesel[MAX_PARTSLIST_NUM_LEN];
  char totobj[MAX_PARTSLIST_NUM_LEN], totobjsel[MAX_PARTSLIST_NUM_LEN];
  char totlamp[MAX_PARTSLIST_NUM_LEN], totlampsel[MAX_PARTSLIST_NUM_LEN];
  char tottri[MAX_PARTSLIST_NUM_LEN];
  char totgplayer[MAX_PARTSLIST_NUM_LEN], totgpframe[MAX_PARTSLIST_NUM_LEN];
  char totgpstroke[MAX_PARTSLIST_NUM_LEN], totgppoint[MAX_PARTSLIST_NUM_LEN];
} SceneStatsFmt;
...
/* Statistics displayed in partslist header. Called regularly on scene changes. */
...
{
#define MAX_PARTSLIST_MEM_LEN 64
  ...
  char memstr[MAX_PARTSLIST_MEM_LEN];
  char gpumemstr[MAX_PARTSLIST_MEM_LEN] = "";
...
#undef SCENE_STATS_FMT_INT
  ...
  ofs = BLI_snprintf(memstr, MAX_PARTSLIST_MEM_LEN, IFACE_(" | Mem: %s"), formatted_mem);

  if (mmap_in_use) {
    BLI_str_format_byte_unit(formatted_mem, mmap_in_use, true);
    BLI_snprintf(memstr + ofs, MAX_PARTSLIST_MEM_LEN - ofs, IFACE_(" (%s)"), formatted_mem);
  }
  ...
  if (GPU_mem_stats_supported()) {
    ...
    ofs = BLI_snprintf(gpumemstr, MAX_PARTSLIST_MEM_LEN, IFACE_(" | Free GPU Mem: %s"), formatted_mem);

    if (gpu_tot_memory) {
      BLI_str_format_byte_unit(formatted_mem, gpu_tot_memory, true);
      BLI_snprintf(gpumemstr + ofs, MAX_PARTSLIST_MEM_LEN - ofs, IFACE_("/%s"), formatted_mem);
    }
  }

  s = stats->partsliststr;
  ofs = 0;

  if (object_mode == OB_MODE_OBJECT) {
    ofs += BLI_snprintf(s + ofs,
                        MAX_PARTSLIST_LEN - ofs,
                        "%s | ",
                        BKE_collection_ui_name_get(layer_collection->collection));
  }

  if (ob) {
    ofs += BLI_snprintf(s + ofs, MAX_PARTSLIST_LEN - ofs, "%s | ", ob->id.name + 2);
  }

  if (obedit) {
    if (BKE_keyblock_from_object(obedit)) {
      ofs += BLI_strncpy_rlen(s + ofs, IFACE_("(Key) "), MAX_PARTSLIST_LEN - ofs);
    }
    ...
    ofs += BLI_strncpy_rlen(s + ofs, memstr, MAX_PARTSLIST_LEN - ofs);
    ofs += BLI_strncpy_rlen(s + ofs, gpumemstr, MAX_PARTSLIST_LEN - ofs);
  }
  ...
  ofs += BLI_snprintf(s + ofs, MAX_PARTSLIST_LEN - ofs, " | %s", versionstr);
#undef MAX_PARTSLIST_MEM_LEN
}

#undef MAX_PARTSLIST_LEN

void ED_partslist_stats_clear(ViewLayer *view_layer)
{
  if (view_layer->stats) {
...
const char *ED_partslist_stats_string(Main *bmain, Scene *scene, ViewLayer *view_layer)
{
  ...
  return view_layer->stats->partsliststr;
}
